...
// struct lockID uint64

// var counterMu sync.Mutex
// var currID = lockID(1)

// struct identifiable interface {
// 	id() lockID
// }
...
// fn (m *RWMutex) RLocker() sync.Locker {
// 	return (*rlocker)(m)
// }

// fn preLock(skip int, p identifiable, gid int64, checkRecursiveLocking bool) {
// 	lo.preLock(skip, p, gid, checkRecursiveLocking)
// }

// fn postLock(skip int, p identifiable, gid int64) {
// 	lo.postLock(skip, p, gid)
// }

// fn postUnlock(p identifiable) {
// 	lo.postUnlock(p)
// }

// fn checkRecursiveLocking(skip int, p identifiable, gid int64) {
// 	lo.checkRecursiveLocking(skip, p, gid)
// }

// fn checkLockOrdering(skip int, p identifiable, gid int64) {
// 	lo.checkLockOrdering(skip, p, gid)
// }

// fn lock(lockFn fn(), ptr identifiable, preLockCheckRecursiveLocking bool) {
// 	if Opts.Disable {
// 		lockFn()
// 		return
// 	}
...
// 				}
// 				Opts.mu.Lock()
// 				println!(Opts.LogBuf, header)
// 				println!(Opts.LogBuf, "Previous place where the lock was grabbed")
// 				fmt.Fprintf(Opts.LogBuf, "goroutine %v lock %p\n", prev.gid, ptr)
// 				printStack(Opts.LogBuf, prev.stack)
// 				println!(Opts.LogBuf, "Have been trying to lock it again for more than", Opts.DeadlockTimeout)
// 				fmt.Fprintf(Opts.LogBuf, "goroutine %v lock %p\n", gid, ptr)
...
// 						println!(Opts.LogBuf, "Here is what goroutine", prev.gid, "doing now")
// 						Opts.LogBuf.Write(g)
// 						println!(Opts.LogBuf)
// 					}
// 				}
...
// 					println!(Opts.LogBuf, "All current goroutines:")
...
// fn (l *lockOrder) postLock(skip int, p identifiable, gid int64) {
// 	stack := callers(skip)
// 	l.mu.Lock()
...
// fn (l *lockOrder) printRecursiveLocking(currentGoRoutineID int64, otherLockID lockID, currentStack []uintptr, otherStack []uintptr, p identifiable) {
// 	Opts.mu.Lock()
// 	println!(Opts.LogBuf, header, "Recursive locking:")
// 	fmt.Fprintf(Opts.LogBuf, "current goroutine %d lock %x\n", currentGoRoutineID, otherLockID)
// 	printStack(Opts.LogBuf, currentStack)
// 	println!(Opts.LogBuf, "Previous place where the lock was grabbed (same goroutine)")
...
// fn (l *lockOrder) printLockOrdering(currentGoRoutineID int64, otherLockID lockID, currentStack []uintptr, otherStack []uintptr, p identifiable, s ss) {
// 	Opts.mu.Lock()
// 	println!(Opts.LogBuf, header, "Inconsistent locking. saw this ordering in one goroutine:")
// 	println!(Opts.LogBuf, "happened before")
// 	printStack(Opts.LogBuf, s.before)
// 	println!(Opts.LogBuf, "happened after")
// 	printStack(Opts.LogBuf, s.after)
// 	println!(Opts.LogBuf, "in another goroutine: happened before")
// 	printStack(Opts.LogBuf, otherStack)
// 	println!(Opts.LogBuf, "happened after")
// 	printStack(Opts.LogBuf, currentStack)
// 	l.other(p)
...
// fn (l *lockOrder) checkRecursiveLocking(skip int, p identifiable, gid int64) {
// 	l.mu.Lock()
// 	defer l.mu.Unlock()
// 	lockID := p.id()
...
// 	}
// }
// fn (l *lockOrder) checkLockOrdering(skip int, p identifiable, gid int64) {
...
// fn (l *lockOrder) storeLockOrder(skip int, p identifiable, gid int64) {
// 	if Opts.DisableLockOrderDetection {
// 		return
// 	}
...
// fn (l *lockOrder) preLock(skip int, p identifiable, gid int64, checkRecursiveLocking bool) {
...
// fn (l *lockOrder) pruneLockOrder(p identifiable) {
...
// fn (l *lockOrder) postUnlock(p identifiable) {
// 	l.mu.Lock()
// 	delete(l.cur, p.id())
// 	l.pruneLockOrder(p)
// 	l.mu.Unlock()
...
// struct rlocker RWMutex

// fn (r *rlocker) Lock()   { (*RWMutex)(r).RLock() }
// fn (r *rlocker) Unlock() { (*RWMutex)(r).RUnlock() }

// // Under lo.mu Locked.
// fn (l *lockOrder) other(ptr identifiable) {
// 	empty := true
// 	for k := range l.cur {
// 		if k == ptr.id() {
// 			continue
...
// 		return
// 	}
// 	println!(Opts.LogBuf, "Other goroutines holding locks:")
