...
impl fmt::Display for CliValidators {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        fn write_vote_account(
            ...
            writeln!(
                ...
                if validator.activated_stake > 0 {
                    format!(
                        "{} ({:.2}%)",
                        build_balance_message(validator.activated_stake, use_tock_unit, true),
        ...
        writeln_name_value(
            f,
            "Active Stake:",
            &build_balance_message(self.total_active_stake, self.use_tock_unit, true),
        )?;
        if self.total_delinquent_stake > 0 {
            writeln_name_value(
                ...
                &format!(
                    "{} ({:0.2}%)",
                    &build_balance_message(self.total_current_stake, self.use_tock_unit, true),
...
pub struct CliNonceAccount {
    ...
    pub tock_per_signature: Option<u64>,
...
impl fmt::Display for CliNonceAccount {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln!(
            f,
            "Balance: {}",
            build_balance_message(self.balance, self.use_tock_unit, true)
        )?;
        writeln!(
            f,
        ...
        if let Some(fees) = self.tock_per_signature {
...
impl fmt::Display for CliStakeState {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln!(
            f,
            "Balance: {}",
            build_balance_message(self.account_balance, self.use_tock_unit, true)
        )?;
        ...
        if let Some(rent_exempt_reserve) = self.rent_exempt_reserve {
            writeln!(
                f,
                "Rent Exempt Reserve: {}",
                build_balance_message(rent_exempt_reserve, self.use_tock_unit, true)
            )?;
        }
            ...
            CliStakeType::Stake => {
                ...
                if show_delegation {
                    ...
                    writeln!(
                        f,
                        "Delegated Stake: {}",
                        build_balance_message(delegated_stake, self.use_tock_unit, true)
                    )?;
                    if self
                        .deactivation_epoch
                        ...
                        .unwrap_or(true)
                    {
                        ...
                        writeln!(
                            f,
                            "Active Stake: {}",
                            build_balance_message(active_stake, self.use_tock_unit, true),
                        )?;
...
impl fmt::Display for CliStakeHistory {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let config = BuildBalanceMessageConfig {
            use_tock_unit: self.use_tock_unit,
            show_unit: false,
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for account in &self.accounts {
            writeln!(
                ...
                "{:<44}  {}",
                account.address,
                &format!("{} TOCK", tock_to_anlog(account.tock))
            )?;
        }
        Ok(())
    }
...
impl fmt::Display for CliSupply {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln_name_value(f, "Total:", &format!("{} TOCK", tock_to_anlog(self.total)))?;
        writeln_name_value(
            f,
            "Circulating:",
            &format!("{} TOCK", tock_to_anlog(self.circulating)),
        )?;
        writeln_name_value(
            f,
            "Non-Circulating:",
            &format!("{} TOCK", tock_to_anlog(self.non_circulating)),
        )?;
...
impl fmt::Display for CliFeesInner {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln_name_value(
            f,
            "Tocks per signature:",
            &self.tock_per_signature.to_string(),
        )?;
...
impl fmt::Display for CliUpgradeableProgram {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln_name_value(
            f,
            "Balance:",
            &build_balance_message(self.tock, self.use_tock_unit, true),
        )?;
        Ok(())
    }
}
        ...
        for program in self.programs.iter() {
            writeln!(
                ...
                &format!(
                    ...
                    program.authority,
                    build_balance_message(program.tock, self.use_tock_unit, true)
                )
            )?;
        }
        Ok(())
        ...
        for buffer in self.buffers.iter() {
            writeln!(
                ...
                &format!(
                    ...
                    buffer.address,
                    buffer.authority,
                    build_balance_message(buffer.tock, self.use_tock_unit, true)
                )
            )?;
        }
        Ok(())
